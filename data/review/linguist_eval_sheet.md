# Hasidic Yiddish G2P — Expert Evaluation Sheet (v2)

**For the reviewer:** below are 10 real sentences from contemporary Hasidic
podcast speech (Williamsburg/KJ/Monsey variety), each followed by the
grapheme-to-phoneme system's pronunciation output — including the embedded
Hebrew quotes, which the system now reads in full. Please judge the output as
a whole: mark anything a native speaker of this variety would say differently,
and write your own reading for it — any romanization is fine, rhyme-words
welcome ("like *father*", "like *put*").

**Phone key:** `a` f**a**ther (short) · `aː` same, held long · `ɛ` b**e**d ·
`ə` schwa · `i` b**ee**t/b**i**t · `u` p**u**t/b**oo**t · `ɔ` l**aw** (NY) ·
`ej` s**ay** · `aj` b**uy** · `ɔj` b**oy** · `oʊ` g**oa**t · `x` ba**ch** ·
`ʃ` **sh** · `ʦ` **ts** · `ʧ` **tsh** · `ʤ` **j** · `ˈ` stress, placed just
before the stressed vowel.

---

**1.**
> דא שטייט וברכתי את לחמך ואת מימיך, דער אויבערשטער בענטשט אים מיט ברויט אין דיינע געדערן.

System: `du ʃtajt uvajrˈaxsi ɛs lˈaxməxu vɛs mˈimix, dɛr ˈɔjbərʃtər bɛnʧt ejm mit brɔjt in dˈaːnə ɡədˈɛrn.`

**2.**
> אז ער וויל עס אמתדיג נעמען די תורה לשמה.

System: `az ɛr vil ɛs ˈɛməzdiɡ nˈɛmən də tˈɔjrə ləʃˈamu.`

**3.**
> אויך האט ראש חודש די יארצייט פון דעם הייליגן רבי ברוכל גארליצער.

System: `oʊx hut rɔjʃ xˈɔjdəʃ də jˈurʦajt fin dejm hˈajliɡn rˈɛbə brixl ɡˈarliʦər.`

**4.**
> קוק אויף משה רבינו, ווי היינט איז זי!

System: `kik oʊf mˈɔjʃə rbˈini, vi haːnt iz zi!`

**5.**
> אז דער בעל הבית פון דער הויז האט געהאט איינשטאלט.

System: `az dɛr bˈaləbus fin dɛr hoʊz hut ɡəhˈat ˈajnʃtalt.`

**6.**
> איז האָבן זיי געקענט נאָך מער לערנען ווי דער דיין.

System: `iz hubn zaj ɡəkˈɛnt nux mɛr lˈirnən vi dɛr daːn.`

**7.**
> זאגט דער הייליגער מאור עיניים, משה רבנוס יארצייט איז אייביג פרשת תצוה, יא?

System: `zuɡt dɛr hˈajliɡər mir ˈajnajm, mˈɔjʃə rbnis jˈurʦajt iz ˈajbiɡ pˈarʃis təʦˈavɛ, ju?`

**8.**
> ס'איז געווען געקענט, האָט מען גערופן טאַקע דעם דיין אַז מען גייט עס קאַשערן.

System: `siz ɡəvˈejn ɡəkˈɛnt, hut mɛn ɡərˈifn tˈakə dejm daːn az mɛn ɡajt ɛs kˈaʃərn.`

**9.**
> און דער סקווערער רבי זכרונו לברכה מיט ר' משה גאַבאַ האָבן געשפּירט אַז מען דאַרף.

System: `in dɛr skvˈɛrər rˈɛbə zixrˈɔjni livrˈuxə mit rɛb mˈɔjʃə ɡˈaba hubn ɡəʃpˈirt az mɛn daf.`

**10.**
> איז ווען די יידן האבן געזאגט נעשה ונשמע, גלייך האט דער אייבערשטער געזאגט ויקחו לי תרומה.

System: `iz vɛn də jidn hubn ɡəzˈuɡt nˈɛʃə ˈinʃmə, ɡlaːx hut dɛr ˈajbərʃtər ɡəzˈuɡt vˈikxi li trˈimə.`

---

*Sentences are verbatim corpus text. System output regenerated 2026-08-08
(engine with MWE fusion, register-aware Hebrew, the full rescue chain, and
PhoneticXeus audio-verified consonant/vowel tables — jˈurʦajt reflects the
23/29-clip audio vote on יארצייט).*
